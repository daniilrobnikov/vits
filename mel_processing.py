...
def spec_to_mel_torch(spec, n_fft, num_mels, sampling_rate, fmin, fmax):
    ...
    if fmax_dtype_device not in mel_basis:
        mel_transform = T.MelScale(n_mels=num_mels, sample_rate=sampling_rate, f_min=fmin, f_max=fmax, n_stft=n_fft // 2 + 1, dtype=spec.dtype).to(spec.device)
        mel_basis[fmax_dtype_device] = mel_transform

    spec = torch.matmul(mel_basis[fmax_dtype_device].fb, spec)
...
# TODO: remove librosa dependency
# import librosa
